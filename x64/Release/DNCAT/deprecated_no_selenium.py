...
class ChangePriceRequest:
    def __init__(self):
        self.session = 0
        self.loginUrl = "https://login.noon.partners/_svc/auth-v1/auth/signin"
        self.refresh()

    def refresh(self):
        self.session = requests.session()
        print(self.session.cookies.get_dict())  # 先打印一下，此时一般应该是空的。

        login_header = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.100 Safari/537.36",
            "Referer": "https://login.noon.partners/en/",
        ...
        json_data = {"email": "buymore@ccsc.sa", "password": "Ccsc2019"}
        self.session.headers.update(login_header)
        self.session.verify = "./noon.cer"
        res = self.session.post(self.loginUrl, data=json_data)
    ...
    def test1(self):
        test1_header = {
            'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.100 Safari/537.36",
            "Referer": "https://core.noon.partners/en-sa/",
            "x-locale": "en-sa"
        }
        self.session.headers.update(test1_header)
        res = self.session.get("https://core.noon.partners/_svc/slapi-v1/seller")
        print(res.status_code)
        print(res.text)


a = ChangePriceRequest()
a.test1()
